...
call readData(un, dt, boxSize, cutOff, nPartDim, T, eps, sig, nSteps, seed)
nPart = nPartDim**3
...
! d'una cel·la cúbica simple. La segona subrutina altera una mica aquesta
! geometria
...
! en coordenades polars, ja que es mes eficient
call boxMuller_polar(seed, vel, nPart, T)
...
do i = 1, nSteps, 1
        ...
        if (trjCount == 100) then
                ...
                write(dataUn,*) time, V, KE, Tinst
                trjCount = 0
        end if
        if (thermCount == 10) then
                call andersen_thermo(dt, T, nPart, i, vel, eps)
                thermCount = 0
        end if
        trjCount = trjCount + 1
